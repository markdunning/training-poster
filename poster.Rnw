...
\SweaveOpts{concordance=TRUE}
  ...
  \begin{frame}{}
            ...
            \usebeamercolor{title in headline}{\color{crukpink}\textbf{\large{http://bioinformatics-core-shared-training.github.io/}}\\[3ex]}
...
Our statistics teaching is done using the R Commander GUI; a user-friendly way of doing statistical analysis in R (\cite{Rcmdr}). Therefore the focus of this training is on the statistical concepts, rather than wrestling with the R language. In this course we introduce the basics of statistics and hypothesis testing so that participants can conduct their own simple statistical testing, and be able to interpret published results. 
